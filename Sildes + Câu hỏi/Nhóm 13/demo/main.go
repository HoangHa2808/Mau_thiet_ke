...
func main() {
	...
	s1 := broker.AddSubscriber("Obama")
	// subscribe BTC and ETH to s1.
	broker.Subscribe(s1, availableTopics["BTC"])
	broker.Subscribe(s1, availableTopics["ETH"])

	// create new subscriber
	//s2 := broker.AddSubscriber("Donald Trump")
	//// subscribe ETH and SOL to s2.
	//broker.Subscribe(s2, availableTopics["ETH"])
	//broker.Subscribe(s2, availableTopics["SOL"])

	go (func() {
		// sleep for 2 sec, and then subscribe for topic DOT for s2
		time.Sleep(8 * time.Second)
		broker.Unsubscribe(s1, availableTopics["ETH"])
	})()

	//go (func() {
	//	// sleep for 3 sec, and then unsubscribe for topic SOL for s2
